...
class Mehrdarman_admissionModelAdmissions extends JModelList
{
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.ordering', 'ordering',
				'a.language', 'language',
				'a.hits', 'hits','ordering', 'state', 'name', 'tel', 'disease', 'description'
			);
		}
	...
	protected function populateState($ordering = 'name', $direction = 'ASC')
	{
		...
		$this->setState('filter.language', $language);		// Set filter state for name
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('a.name LIKE' . $s . ' OR a.tel LIKE' . $s . ' OR a.disease LIKE' . $s . ' OR a.description LIKE' . $s );
			}
		}
